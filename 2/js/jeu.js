...
function create() {
    ...
    const infoTexte = this.add.text(screenCenterX, screenCenterY - 370, "Choisir une catégorie de question", { fontFamily: "FFFTusj", fontSize: 30, color: ' #ffffff ', wordWrap: { width: window.innerWidth }}).setOrigin(0.5);
    ...
    Phaser.Actions.SetXY(groupBtn.getChildren(), screenCenterX, screenCenterY / 2, 0, 60);
...
function commencerQuestion(categorie, self) {
    ...
    //!2
    score = self.add.text(10, 10, `Question: 1/${Object.keys(quiz[categorie]).length}`, { fontFamily: "FFFTusj", fontSize: 30, color: ' #ffffff ',  wordWrap: { width: window.innerWidth }});
    score.setData('score', 0)
    afficherQuestion(categorieChoisie, self, 0, Object.keys(quiz[categorie]).length)
}

function afficherQuestion(categorieChoisie, scene, index, max) {
    if ((max - index) == 0) return finDePartie(scene, max)
    groupBtn.clear();
    score.text = ""
    score.text = `Question: ${index + 1}/${max}`
...
function finDePartie(scene, max) {
    //!4
    //affiche score
    const text_score = scene.add.text(scene.screenCenterX, scene.screenCenterY, `Votre score ${score.getData('score')}/` + max, { fontFamily: "FFFTusj", fontSize: 30, color: ' #ffffff ', wordWrap: { width: window.innerWidth }}).setOrigin(0.5, 0.5);
    //Redémarre la scene au clique
    const rejouer = scene.add.text(scene.screenCenterX, scene.screenCenterY + 200, "Rejouer",
    {
        fontFamily: "FFFTusj",
        fontSize: 30,
        color: ' #ffffff ',
